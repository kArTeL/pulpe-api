...
2. **Match `name` and `description` via Prisma's `contains`, not raw SQL / FTS5.** SQLite's `LIKE` (what `contains` compiles to) is already case-insensitive for ASCII by default. Prisma's `mode: 'insensitive'` is **not supported on the SQLite connector** and throws `Unknown argument 'mode'` at runtime — it must not be used. Plain `where: { OR: [{ name: { contains: q } }, { description: { contains: q } }] }`, no raw SQL, per AGENTS.md's ban on hand-written interpolated SQL.
